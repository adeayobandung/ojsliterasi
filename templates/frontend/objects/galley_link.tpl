...
<a class="galley-link btn-sm btn-block btn {if $isSupplementary}btn-default{else}btn-primary{/if} {$type}" role="button" href="{url|escape page=$page op="view" path=$parentId|to_array:$galley->getBestGalleyId($currentJournal)}">
	...
	<span class="fa fa-file-pdf-o"></span> {$galley->getGalleyLabel()|escape}
