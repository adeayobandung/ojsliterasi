...
<div id="main-content" class="page page_login">
	...
	<form class="pkp_form login" id="login" method="post" action="{$loginUrl}">
		...
		<div class="form-group">
			<label for="login-password"><i class="fa fa-lock"></i>
		...
		<div class="buttons">
			<button type="submit" class="btn btn-primary">
				<i class="fa fa-unlock"></i> {translate key="user.login"}
			</button>

			{if !$disableUserReg}
				{capture assign="registerUrl"}{url page="user" op="register" source=$source}{/capture}
				<a class="btn btn-default register-button" href="{$registerUrl}" role="button">
					<i class="fa fa-keyboard-o"></i> {translate key="user.login.registerNewAccount"}
				</a>
			{/if}
		</div>
	</form>
